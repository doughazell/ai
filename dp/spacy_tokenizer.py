...
class StreamSpacyTokenizer(Component):
    ...
    def __call__(self, batch: Union[List[str], List[List[str]]]) -> Union[List[List[str]], List[str]]:
        ...
        if isinstance(batch[0], str):
            if self.lemmas:
                StreamSpacyTokenizer.lemmaList = list(self._lemmatize(batch))
                print("StreamSpacyTokenizer lemma str: ",StreamSpacyTokenizer.lemmaList)

                return StreamSpacyTokenizer.lemmaList
            
                #return list(self._lemmatize(batch))
            else:
                tokenList = list(self._tokenize(batch))
                print("StreamSpacyTokenizer token str: ",tokenList)
                return tokenList
            
                #return list(self._tokenize(batch))
        if isinstance(batch[0], list):
            batchList = [detokenize(doc) for doc in batch]
            print("StreamSpacyTokenizer list: ",batchList)
            return batchList
