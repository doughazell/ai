...
class SQLiteDataIterator(DataFittingIterator):
    ...
    def __init__(self, load_path: Union[str, Path], batch_size: Optional[int] = None,
        ...
        # 18/12/23 DH: Needless Russian obfuscation of ids being from 14GB DB rather than 'tfidf'
        logger.info("*** No longer getting entire DB ID's and just making 'doc_ids' an empty list ***")
        self.doc_ids = []
    ...
    def get_doc_ids(self) -> List[Any]:
        ...
        # 15/12/23 DH:
        record_num = 1
        logger.info("*** Only getting {} records ***".format(record_num))
        ids = cursor.fetchmany(record_num)
