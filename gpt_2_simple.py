...
def finetune(sess,
    ...
    # 1/1/24 DH:
    print("Getting 'tf_sample', used in 'generate_samples()', from 'sample_sequence()' in 'finetune()'")
    ...
    # 4/1/24 DH: Print out the GPT-2 model
    ckptFile = os.path.join("checkpoint/run1", "model-33")
    print("ckptFile: ",ckptFile)
    
    reader = tf.compat.v1.train.NewCheckpointReader(ckptFile)

    # 6/1/24 DH: https://github.com/tensorflow/tensorflow/blob/master/tensorflow/python/tools/inspect_checkpoint.py#L54
    from tensorflow.python.tools.inspect_checkpoint import print_tensors_in_checkpoint_file
    print("print_tensors_in_checkpoint_file()")
    print_tensors_in_checkpoint_file(file_name=ckptFile, all_tensors=True, tensor_name='')
    ...
    # 2/1/24 DH: =========================== STOP ==================================
    print("------------------------------------")
    for i in range(len(data_sampler.chunks)):
        print(i,")",data_sampler.chunks[i])

        chunkSeg = data_sampler.chunks[i]

        #text = enc.decode(out[i])
        text = enc.decode(chunkSeg)
        print("enc.decode(chunkSeg): ",text)

        # 2/1/24 DH: Tokens are codes in 'datasampler.chunks'
        print("===")
        for j in range(len(chunkSeg)):
            print("Token",j,"(",chunkSeg[j],"):",enc.decoder[chunkSeg[j]])

    #text = (gen_text.split("Ċ"))[0]
    huffCodes = data_sampler.chunks[0]
    print()
    print("The Huff: ",huffCodes.__class__, huffCodes)
    
    itemIndices = np.where(huffCodes == 198)
    huffSplit = np.split(huffCodes, itemIndices[0])
    text = enc.decode(huffSplit[0])

    print("itemIndices: ", itemIndices,"huffSplit[0]: ",huffSplit[0],", text: ",text)

    print()
    text = "Hello"
    print(text)
    pairs = encoder.get_pairs(text)
    print("encoder.get_pairs():",pairs.__class__, pairs)
    print("------------------------------------")
    ...
    def sample_batch():
        # 2/1/24 DH:
        print("PREVENTING: AssertionError: Dataset files are too small to sample 1024 tokens at a time")
        print("CHANGED TO: 'data_sampler.sample(sampleLen)'")

        #return [data_sampler.sample(1024) for _ in range(batch_size)]
        sampleLen = len(data_sampler.chunks)
        return [data_sampler.sample(sampleLen) for _ in range(batch_size)]
...
def generate(sess,
    ...
    while generated < nsamples:
        if not prefix:
            # 1/1/24 DH:
            print("Running sess without a prefix with: ",output.__class__)
            
            # "The North the south,"
            #out = [[ 198, 464, 2258, 262, 5366, 11]]
            # "The North the south"
            #out = [[ 198, 464, 2258, 262, 5366]]
            # "The North"
            # 'encoder.json': "\u0120North": 2258, "The": 464, "\u010a": 198
            out = [[ 464, 2258]]
            
            #print("COMMENTED-OUT 'sess.run()'")
            out = sess.run(output)
            print("out:",out.__class__,", shape:",out.shape)
        else:
            # 1/1/24 DH:
            print("Running sess with 'feed_dict' defined in 'sess.run()'")
        ...
        for i in range(batch_size):
            ...
            if not return_as_list and not destination_path:
                # 1/1/24 DH:
                print("*** Printing since no, 'return_as_list' or no, 'destination_path' ***")
                print("----------------------")
                print("Generated ({}): '{}'".format(len(gen_text), gen_text))

                tokens = np.stack(enc.encode(gen_text))
                itemIndices = np.where(tokens == 198)
                huffSplit = np.split(tokens, itemIndices[0])

                text = enc.decode(huffSplit[0])
                print( "Text: ", text)
                print("----------------------")
                tokenNum = len(tokens)
                print("Tokens ({}):".format(tokenNum))
                print("------------")
                for k in range(tokenNum):
                    print("Token",k,"(",tokens[k],"):",enc.decoder[tokens[k]])
