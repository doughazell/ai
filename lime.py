...
class Lime(object):
  ...
  def getSegmentedPrediction(self, img=None):
    self.getImagePrediction(img=img)
    self.getTopPredictions()
    self.segmentImage()
  
  # 13/9/23 DH:
  def getCoefficientsFromMaskedPredictions(self):
    # Step 1/4
    self.createRandomPertubations()
    # Step 2/4
    self.getPredictions()
    # Step 3/4
  ...
  def getImagePrediction(self, img=None):
  ...
  def getTopPredictions(self):
    ...
    # '[-5:]' ie take last 5 elements (from 'argsort()' which has highest last with default ‘quicksort’)
    # '[::-1]' ie reverse order of elements using 'slice([start:stop:step])'
    
    # 11/9/23 DH: https://numpy.org/doc/stable/reference/generated/numpy.argsort.html
  ...
  def segmentImage(self):
    ...
    #   (ie pixel values the same as segment ID, hence segments of same value that need to be collated with 
    #    'np.unique()')
